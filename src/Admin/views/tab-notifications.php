...
<div class="waxap-section-header">
	<h2><?php esc_html_e( '¿En qué momentos avisas a tus clientes?', 'waxap-for-woocommerce' ); ?></h2>
	<p><?php esc_html_e( 'Activa los estados que enviarán un WhatsApp. Usa el lápiz para personalizar el mensaje.', 'waxap-for-woocommerce' ); ?></p>
</div>
...
<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" autocomplete="off">
	...
	<div class="wan-field-rows" style="max-width:480px;margin-bottom:28px;">
		<div class="wan-field-row">
			<label for="wan-country-code" class="wan-field-label">
				<?php esc_html_e( 'Prefijo de país (teléfonos de clientes)', 'waxap-for-woocommerce' ); ?>
				<span class="wan-field-hint"><?php esc_html_e( 'Sin el +. Se añade si el número del cliente no lo lleva. España: 34', 'waxap-for-woocommerce' ); ?></span>
			</label>
	...
	<div class="wan-status-list">
		<?php
		foreach ( $statuses as $waxap_key => $waxap_status_info ) :
			$waxap_is_enabled  = in_array( $waxap_key, $enabled_statuses, true );
			$waxap_input_id    = 'wan-status-' . esc_attr( $waxap_key );
			$waxap_textarea_id = 'wan-tpl-' . esc_attr( $waxap_key );
			$waxap_panel_id    = 'wan-tpl-panel-' . esc_attr( $waxap_key );
			?>
		<div class="wan-status-item">

			<div class="wan-status-card-row">
				<label class="wan-status-card" for="<?php echo esc_attr( $waxap_input_id ); ?>">
					<span class="wan-status-dot-indicator"
							style="background-color: <?php echo esc_attr( $waxap_status_info['color'] ); ?>;"></span>

					<div class="wan-status-info">
						<strong><?php echo esc_html( $waxap_status_info['label'] ); ?></strong>
						<?php if ( ! empty( $waxap_status_info['desc'] ) ) : ?>
						<span><?php echo esc_html( $waxap_status_info['desc'] ); ?></span>
						<?php endif; ?>
					</div>
					...
					<div class="wan-toggle">
						<input
							type="checkbox"
							id="<?php echo esc_attr( $waxap_input_id ); ?>"
							name="wan_notify_statuses[]"
							value="<?php echo esc_attr( $waxap_key ); ?>"
							class="wan-toggle-input"
							<?php checked( $waxap_is_enabled ); ?>
						>
				...
				<button
					...
					data-panel="<?php echo esc_attr( $waxap_panel_id ); ?>"
					data-has-content="<?php echo ! empty( $templates[ $waxap_key ] ) ? '1' : '0'; ?>"
					aria-expanded="false"
					aria-label="<?php esc_attr_e( 'Editar plantilla de mensaje', 'waxap-for-woocommerce' ); ?>"
					title="<?php esc_attr_e( 'Editar mensaje', 'waxap-for-woocommerce' ); ?>"
				>
			...
			<div class="wan-tpl-panel" id="<?php echo esc_attr( $waxap_panel_id ); ?>">
				<div class="wan-tpl-panel-inner">
					<textarea
						name="wan_templates[<?php echo esc_attr( $waxap_key ); ?>]"
						id="<?php echo esc_attr( $waxap_textarea_id ); ?>"
						class="wan-template-textarea wan-template-textarea--inline"
						rows="3"
					><?php echo esc_textarea( $templates[ $waxap_key ] ?? '' ); ?></textarea>
					<div class="wan-template-footer">
						<?php foreach ( [ '{nombre}', '{pedido}', '{estado}', '{total}', '{enlace}' ] as $waxap_var ) : ?>
						<span class="wan-var-chip" data-target="<?php echo esc_attr( $waxap_textarea_id ); ?>"><?php echo esc_html( $waxap_var ); ?></span>
						<?php endforeach; ?>
						<span class="wan-char-count" id="wan-count-<?php echo esc_attr( $waxap_key ); ?>">0</span>
					</div>
				</div>
			</div>
